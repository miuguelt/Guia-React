...
.SYNOPSIS
    ...
    Compatible con dashboard DevBrain (-Daemon, -Stop, -Status).
.DESCRIPTION
    ...
    TOKENS:  Guia React 8030 web 35731
#>
...
# Determina ejecutable: %APPDATA% no siempre apunta al perfil que tiene el
# shim global cuando el proyecto se lanza desde el dashboard, y el fallback a
# npx.cmd falla con MODULE_NOT_FOUND. Se prueban rutas reales en orden.
$lsCandidates = @(
    "$env:APPDATA\npm\live-server.cmd",
    "$env:USERPROFILE\AppData\Roaming\npm\live-server.cmd",
    (Get-Command live-server.cmd -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty Source)
) | Where-Object { $_ -and (Test-Path -LiteralPath $_) }

$lsExe = if ($lsCandidates) { $lsCandidates[0] } else { "npx.cmd" }
...
# Respaldo: lanzado desde el dashboard, el proceso no siempre ve el shim global
# de npm y npx.cmd aborta con MODULE_NOT_FOUND, dejando la guía sin servir.
# Python sí está disponible en todos los contextos: se pierde el livereload,
# pero el sitio carga en el puerto declarado.
if (-not $nodeConn) {
    Write-Host "⚠️  live-server no abrió el puerto; usando python -m http.server." -ForegroundColor Yellow
    $fallbackBat = "$LogDir\run_httpserver.bat"
    @"
@echo off
cd /d "$SiteDir"
python -m http.server $Port --bind 127.0.0.1 > "$LogFile" 2> "$ErrFile" < NUL
"@ | Out-File -FilePath $fallbackBat -Encoding ascii -Force

    Start-Process -FilePath "cmd.exe" -ArgumentList "/c `"$fallbackBat`"" `
        -WorkingDirectory $SiteDir -WindowStyle Hidden | Out-Null
...
# El dashboard DevBrain puede usar -Status para verificar el puerto.
